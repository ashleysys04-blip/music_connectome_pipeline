...
구체적으로, mean clustering coefficient는 `clustering_coef_wu`, modularity(Q)는 Louvain community detection(`modularity_louvain_und`), global efficiency는 `efficiency_wei`(평균 역최단거리)로 계산하였다. characteristic path length는 weight를 connection length로 변환(1/weight)한 뒤 `distance_wei`(Dijkstra)로 최단거리 행렬을 구하고 `charpath`로 평균하여 산출하였다. small-worldness(σ)는 단일 BCT measure가 아니라, degree를 보존한 채 무작위화한 null model(`randmio_und`)에 대해 clustering coefficient와 characteristic path length를 정규화한 σ = (C/C_rand)/(L/L_rand)로 계산하였다. within/between-network 강도는 정규화된 weight 행렬에서 해당 ROI 블록의 평균으로, total_strength는 `strengths_und`로 산출하였다. 모든 행렬은 WU(weighted undirected) network로 처리되었다.
