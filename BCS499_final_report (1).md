...
Specifically, the mean clustering coefficient was computed with `clustering_coef_wu`, modularity (Q) with Louvain community detection (`modularity_louvain_und`), and global efficiency with `efficiency_wei` (mean inverse shortest path). Characteristic path length was obtained by mapping weights to connection lengths (1/weight), computing the shortest-path distance matrix with `distance_wei` (Dijkstra), and averaging it via `charpath`. Small-worldness (σ) is not a single BCT measure; it was computed as σ = (C/C_rand)/(L/L_rand), normalizing clustering coefficient and characteristic path length against degree-preserving randomized null models (`randmio_und`). The within/between-network strengths are means over the corresponding ROI blocks of the normalized weight matrix, and total_strength was computed with `strengths_und`. All matrices were treated as WU (weighted undirected) networks.
